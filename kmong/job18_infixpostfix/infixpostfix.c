/*
	 ...
	 그냥 추가함 chk flag를 이용해 연속된 숫자인지 판단함
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
char* infix_to_postfix(char* str){
	int len = strlen(str); 
	char* ret = (char*)malloc(len*2); // 동적할당 !
	int retlen=0;
	...
	for (int i=0; i<len; i++){ // S문자열 을 순회!
		if (oper(str[i])) {
		...
		else if(str[i] == '(') s[++top] = str[i], chk=0; //S[i] 가 괄호일경우
			...
			if(chk) ret[retlen++]=str[i]; //숫자인 경우
			else { ret[retlen++] = ' '; ret[retlen++]=str[i]; chk=1;}
		}
	}
